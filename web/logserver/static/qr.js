 ...
 * Zero external dependencies, supports UTF-8, binary and standard base64 payloads.
 * Self-contained for offline / air-gapped usage.
 */
(function (root, factory) {
    ...
    // QR Code Model & Generator Implementation (Type Number 1-40, ECC Level L, M, Q, H)
    var QRCode = function (typeNumber, errorCorrectLevel) {
        this.typeNumber = typeNumber || 0;
        this.errorCorrectLevel = errorCorrectLevel || QRCode.CorrectLevel.M;
        this.modules = null;
        this.moduleCount = 0;
        this.dataCache = null;
        this.dataList = [];
    ...
    /**
     * Helper to render QR code directly to a HTML Canvas element.
     */
    QRCode.renderCanvas = function (text, canvas, options) {
        options = options || {};
        var cellSize = options.cellSize || 6;
        var margin = (options.margin !== undefined) ? options.margin : 4;
        var darkColor = options.dark || '#000000';
        var lightColor = options.light || '#ffffff';

        var qr = new QRCode(options.typeNumber || 0, options.correctLevel || QRCode.CorrectLevel.M);
        qr.addData(text);
        qr.make();
