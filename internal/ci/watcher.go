...
const (
	watchPollInterval  = 2 * time.Second // github-workflows.py:103
	uploadDebounce     = 3 * time.Second // github-workflows.py:89
	uploadRetries      = 3
...
// uploadRetryBackoff between attempts. Ported from
// github-workflows.py:69-103 (watch_and_upload_proxy_info()). Blocks until
// ctx is cancelled.
...
func checkAndUpload(ctx context.Context, token, repo, path, branch, tempDir string, state *fileState, log LogFunc) {
	...
	if err != nil {
		return // file doesn't exist yet — same tolerant behavior as the Python version
	}
