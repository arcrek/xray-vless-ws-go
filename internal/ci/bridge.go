...
const bridgeDelay = 5 * time.Hour // github-workflows.py:107
...
// time limit — ported from github-workflows.py:105-110/27-62
// (run_threads()'s bridge timer + bridge_workflows()). It returns
// immediately; the actual dispatch happens on its own goroutine after
// bridgeDelay, or not at all if ctx is cancelled first (e.g. clean shutdown
...
// dispatchWorkflow re-triggers the current workflow, ported from
// github-workflows.py:27-62 (bridge_workflows()). It reads
