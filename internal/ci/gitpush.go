...
// basename if rename is empty), commits, and force-pushes. Shells out to
// the `git` CLI rather than using go-git — see plan decision log #11 for
// the tradeoff (this makes the binary depend on `git` being on PATH,
// accepted since the GitHub Actions runner already has it and go-git's
// force-push/orphan-branch parity carries its own edge-case risk).
//
// A hidden/-prefix branch auto-delete-after-push behavior was considered
// and intentionally NOT implemented — confirmed dead code in this repo (no
// caller ever uses a "hidden/"-prefixed branch), dropped per plan decision
// log #12 (YAGNI, contradicts "don't over-build").
//
// Security note: the token is embedded in the clone URL, which is visible
// in `ps aux` on multi-user systems — a known limitation, not addressed
// here.
...
// checkout, then falls back to creating a fresh orphan branch with an
// empty initial commit (pushed immediately so the branch exists on the
// remote).
