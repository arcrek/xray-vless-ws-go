...
// ParsePorts parses the PORT env var. Supported formats (note: a naive
// `split(":")` behavior — an IPv6 literal like "::1:8888" would split
// incorrectly; this is a known pre-existing limitation, preserved
// intentionally rather than fixed silently, per Phase 1's risk note):
//
//	"8888"              -> 0.0.0.0:8888
...
// TunnelTarget returns the IP/port cloudflared should dial: if the first
// inbound listens on the wildcard address, cloudflared must be pointed at
// 127.0.0.1 instead (it can't dial 0.0.0.0).
