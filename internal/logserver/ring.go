...
// LogLine is one entry in the ring buffer, matching the JSON shape the
// /logs handler returns (id/time/type/text).
type LogLine struct {
	ID   int    `json:"id"`
	Time string `json:"time"`
	Type string `json:"type"` // log source — XRAY/CLOUDFLARE/INFO/etc.
	Text string `json:"text"`
}

// Ring is a bounded, mutex-protected log buffer. No need for anything
// fancier at this log volume (a few lines/sec at most).
type Ring struct {
	mu      sync.Mutex
	maxLogs int
	logs    []LogLine
