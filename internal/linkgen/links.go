...
// BuildLinks builds the vless:// link list, matching main.py:343-372
// (print_vless_links()) field for field and byte for byte (see
// links_test.go's golden comparison against a real Python-generated
// fixture). Every SNIEntry produces two links — TLS on 443, no-TLS on 80 —
// intentionally, not collapsed to one (some carriers only unmeter one port).
//
...
func BuildLinks(cfg *config.Config, uuid, tunnelHost string) []string {
	encodedPath := pyQuote(cfg.WSPath)

	// main.py:347-349: prefer the configured WS_HOST over the detected
	...
	for _, entry := range cfg.FakeSNI {
		encodedRemark := pyQuote(entry.Remark)
