...
func pollReadyOnce(ctx context.Context, client *http.Client, metricsAddr string) ReadyState {
	...
	// rather than erroring, tolerating unexpected responses during startup.
...
// channel is closed when ctx is done. Gives Phase 3's Supervisor an actual
// connectivity signal to act on, instead of a blind fixed-interval sleep
// loop.
