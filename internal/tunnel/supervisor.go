...
// sustained-not-ready — replacing main.py's `while True: ... time.sleep(1)`
// block for the cloudflared half only (xray-core's own lifecycle, Phase 2,
// doesn't need this kind of supervision since it's in-process).
type Supervisor struct {
	...
	// supervisor-generated events (restarts, new hostnames), tagged the
	// same way main.py's monitor_cloudflare() prints them.
	...
	// this keeps the named-tunnel path's zero-log-parsing guarantee even
	// stronger than main.py's original "registered tunnel connection"
	// log-line match).
