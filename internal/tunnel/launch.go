...
// whether cfg.TunnelToken is set, ported from main.py:254-274
// (launch_cloudflared()) with one addition: every launch now gets a
// `--metrics 127.0.0.1:<random-free-port>` flag so PollReady has something
// to poll (the Python PoC never exposed a metrics port at all).
func Launch(cfg *config.Config, binPath, workDir string) (*Handle, error) {
	...
	// Combine stdout+stderr into one stream, same as Python's
	// stderr=subprocess.STDOUT — build the pipe manually (rather than
	// cmd.StdoutPipe()) so both cmd.Stdout and cmd.Stderr can point at the
	// same write end.
